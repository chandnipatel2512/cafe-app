...
def update_product(product_list=[]):
    id, unused = select_item(
        product_list, "number of the product you would like to update"
    )
    index = next(
...
def update_courier(courier_list=[]):
    id, unused = select_item(
        courier_list, "number of the product you would like to update"
    )
    index = next(
...
def update_order_status(order_list=[]):
    id, unused = select_item(
        order_list, "number of the status you would like to update"
    )
    index = next((index for (index, d) in enumerate(order_list) if d["id"] == id), None)
...
def update_basket(transaction_id="", basket_list=[], product_list=[]):
    ...
    while user_input in options:
        ...
        user_input = input(
            "\nEnter 1 to add a product, 2 to update a product, or 3 to delete a product. Alternatively, enter anything else to cancel.\n"
        )
        if user_input == "1":
            ...
            basket_list_index = next(
                (index for (index, d) in enumerate(basket_list) if d["id"] == id), None
            )
